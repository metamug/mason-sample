...
<c:choose>
 ...
 <m:upload/>
 <sql:update var="result" dataSource="jdbc/mtgDataSource">INSERT INTO movie (name,rating) values ('test',1.0);
</sql:update>
 <m:out value="${map}" tableName="movie"/>
 </c:when>

 <c:when test="${empty mtgReq.id and mtgReq.method eq 'DELETE'}">
 <sql:update var="result" dataSource="jdbc/mtgDataSource">TRUNCATE TABLE movie
</sql:update>
 <m:out value="${map}" tableName="movie"/>
 </c:when>
